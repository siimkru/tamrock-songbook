...
This repository is a ChordPro songbook for the Rattalaagri band. Song sources live in `songs/` as `*.cho`; use lowercase, underscore-separated filenames such as `oma_laulu_ei_leia_ma_ules.cho`. Instrument-specific ChordPro configurations are in `config/` (`guitar.json`, `keyboard.json`, and `lyrics.json`). Google Drive utilities live in `scripts/`, and CI build/upload behavior is defined in `.github/workflows/main.yml`. Put generated PDFs in the ignored `build/` directory.
